...
// Start vote timeout when voting begins
function startVoteTimeout() {
  ...
  voteTimeoutId = setTimeout(() => {
    ...
    // Force unanimous decision for back to menu
    const voteState = {
      anotherRound: 0,
...
// Stop vote timeout when voting completes
...
// Helper function to check and send leaderboard
function checkAndSendLeaderboard() {
  ...
  // If all remaining players have submitted scores and leaderboard not sent yet
  if (scoresReceived === totalPlayers && totalPlayers > 0 && !leaderboardSent) {
    leaderboardSent = true;
    
    // Create leaderboard by sorting scores
    ...
    // Start vote timeout when leaderboard is sent
    startVoteTimeout();
...
io.on('connection', (socket) => {
  ...
  socket.onAny((eventName, ...args) => {
    if (!['input', 'position', 'abilityActivation', 'positionCorrection', 'dummyDamage'].includes(eventName)) {
      console.log(`🔍 DEBUG: Received event '${eventName}' from ${socket.id.slice(-4)}:`, args);
    }
  });
  ...
  socket.on('position', (update) => {
    ...
    // Fast movement logging disabled for cleaner output
    // const speed = Math.sqrt(update.velocity.x ** 2 + update.velocity.y ** 2 + update.velocity.z ** 2);
    // if (speed > 8) console.log(`🚀 ${socket.id.slice(-4)}: Fast movement ${speed.toFixed(1)}m/s`); 
  });
  ...
  socket.on('voteAnotherRound', () => {
    ...
    // Remove from backToMenu votes and add to anotherRound
    ...
    // Check for menu priority (any menu vote wins immediately)
    if (voteState.backToMenu > 0) {
      ...
      // Stop vote timeout since decision is reached
      stopVoteTimeout();
  ...
  socket.on('voteBackToMenu', () => {
    ...
    // Remove from anotherRound votes and add to backToMenu
  ...
  socket.on('finalScore', (data) => {
    ...
    // Store player's final score with validation
    ...
    // Check if all remaining players have submitted scores
  ...
  socket.on('disconnect', () => {
    ...
    if (hadScore) {
      ...
      // Check if remaining players can still complete the leaderboard
    ...
    if (hadVote && postRaceVotes.totalPlayers > 0) {
      ...
      // Check for menu priority after disconnection (any menu vote wins immediately)
